...
func TestEveryAny(t *testing.T) {
	even := hiter.Map(func(i int) int { return i * 2 }, hiter.Range(0, 10))
	...
	t.Run("Every", func(t *testing.T) {
		...
		assert.Assert(t, !hiter.Every(isEven, hiter.Concat(even, hiter.Once(5))))
		assert.Assert(t, !hiter.Every2(isEven2, hiter.Concat2(even2, hiter.Once2(5, 2))))
	})
	...
	t.Run("Any", func(t *testing.T) {
		...
		assert.Assert(t, hiter.Any(isEven, hiter.Concat(hiter.Once(7), even, hiter.Once(5))))
		assert.Assert(t, hiter.Any2(isEven2, hiter.Concat2(hiter.Once2(7, 8), even2, hiter.Once2(5, 2))))

		assert.Assert(t, hiter.Any(isOdd, hiter.Concat(even, hiter.Once(5))))
		assert.Assert(t, hiter.Any2(isOdd2, hiter.Concat2(even2, hiter.Once2(5, 5))))
	})
}
