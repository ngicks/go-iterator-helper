...
func TestEveryAny(t *testing.T) {
	even := xiter.Map(func(i int) int { return i * 2 }, hiter.Range(0, 10))
	...
	t.Run("Every", func(t *testing.T) {
		...
		assert.Assert(t, !hiter.Every(isEven, xiter.Concat(even, hiter.Once(5))))
		assert.Assert(t, !hiter.Every2(isEven2, xiter.Concat2(even2, hiter.Once2(5, 2))))
	})
	...
	t.Run("Any", func(t *testing.T) {
		...
		assert.Assert(t, hiter.Any(isEven, xiter.Concat(hiter.Once(7), even, hiter.Once(5))))
		assert.Assert(t, hiter.Any2(isEven2, xiter.Concat2(hiter.Once2(7, 8), even2, hiter.Once2(5, 2))))

		assert.Assert(t, hiter.Any(isOdd, xiter.Concat(even, hiter.Once(5))))
		assert.Assert(t, hiter.Any2(isOdd2, xiter.Concat2(even2, hiter.Once2(5, 5))))
	})
}
