...
func TestSkip(t *testing.T) {
	...
	t.Run("Skip 0 element", func(t *testing.T) {
		testcase.One[int]{
			Seq: func() iter.Seq[int] {
				return hiter.Skip(0, slices.Values(src))
			},
	...
	t.Run("SkipLast 0 element", func(t *testing.T) {
		testcase.One[int]{
			Seq: func() iter.Seq[int] {
				return hiter.SkipLast(-1, slices.Values(src))
			},
			Expected: []int{1, 1, 4, 2, 1, 1, 4, 2, 1, 1, 6, 2, 1, 1, 9, 2},
			BreakAt:  3,
		}.Test(t)
	...
	t.Run("Skip2 0 element", func(t *testing.T) {
		testcase.Two[int, int]{
			Seq: func() iter.Seq2[int, int] {
				return hiter.Skip2(0, slices.All(src))
			},
			Expected: []hiter.KeyValue[int, int]{
				{0, 1},
				{1, 1},
	...
	t.Run("SkipLast2 0 element", func(t *testing.T) {
		testcase.Two[int, int]{
			Seq: func() iter.Seq2[int, int] {
				return hiter.SkipLast2(0, slices.All(src))
			},
			Expected: []hiter.KeyValue[int, int]{
				{0, 1},
				{1, 1},
